...
struct MatchDetailView: View {
    ...
    private func detailTeam(_ team: Team?) -> some View {
        VStack(spacing: 12) {
            ...
            if let team {
                ScorePill(score: ArabicNumberFormatter.localized(match.score(for: team)), isLeading: false)
            }
        }
    ...
    private var mapProgress: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            if let liveState = match.liveState {
                ...
                Text(liveState.phase).font(E360Font.body(14, weight: .bold)).foregroundStyle(E360Color.textPrimary)
                if !match.maps.isEmpty {
                    MatchMapSeriesView(maps: match.maps, teams: match.teams,
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 6) {
                ForEach(1...24, id: \.self) { round in
                    Capsule().fill(round <= currentRound ? E360Color.accent : E360Color.elevatedSurface).frame(height: 12)
                }
            }
...
private struct MatchDetailStatusBanner: View {
    let message: String; let systemImage: String
    var body: some View {
        HStack(spacing: 10) {
            Image(systemName: systemImage).foregroundStyle(E360Color.gold)
            Text(message).font(E360Font.body(12, weight: .semibold)).foregroundStyle(E360Color.textSecondary)
            Spacer(minLength: 0)
        }
        .padding(.horizontal, 12).padding(.vertical, 10)
        .background(E360Color.elevatedSurface, in: RoundedRectangle(cornerRadius: 16, style: .continuous))
        .overlay(RoundedRectangle(cornerRadius: 16, style: .continuous).stroke(E360Color.gold.opacity(0.32), lineWidth: 1))
    }
}
...
private struct MetricPill: View {
    ...
    var body: some View {
        VStack(spacing: 6) {
            Text(title).font(E360Font.body(11, weight: .medium)).foregroundStyle(E360Color.textSecondary).lineLimit(1)
            Text(value).font(E360Font.number(15, weight: .bold)).foregroundStyle(E360Color.textPrimary)
        }
...
private struct MatchMapSeriesView: View {
    ...
    private func mapTitle(for map: MatchMap) -> String {
        ...
        return String(format: String(localized: "match.mapNumber"), ArabicNumberFormatter.localized(map.number))
    }
    ...
    private func halvesText(_ vals: [MatchMapScore]) -> String? {
        ...
        if let a = v[0].firstHalfRounds, let b = v[1].firstHalfRounds {
            parts.append(String(format: String(localized: "match.firstHalfShort"), "\(ArabicNumberFormatter.localized(a))-\(ArabicNumberFormatter.localized(b))"))
        }
        if let a = v[0].secondHalfRounds, let b = v[1].secondHalfRounds {
            parts.append(String(format: String(localized: "match.secondHalfShort"), "\(ArabicNumberFormatter.localized(a))-\(ArabicNumberFormatter.localized(b))"))
        }
        if let a = v[0].overtimeRounds, let b = v[1].overtimeRounds, a > 0 || b > 0 {
            parts.append(String(format: String(localized: "match.overtimeShort"), "\(ArabicNumberFormatter.localized(a))-\(ArabicNumberFormatter.localized(b))"))
        }
