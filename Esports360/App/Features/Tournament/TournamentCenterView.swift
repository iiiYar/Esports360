...
struct TournamentCenterView: View {
    ...
    private var hero: some View {
        ...
        .overlay(RoundedRectangle(cornerRadius: 24, style: .continuous).stroke(E360Color.gold.opacity(0.26), lineWidth: 1))
    }

    // MARK: - Metrics Strip
    private var metricsStrip: some View {
        HStack(spacing: 10) {
            TournamentMetricCard(title: String(localized: "tournament.metric.featured", defaultValue: "المميزة"), value: viewModel.featuredTournaments.count, color: E360Color.gold)
            TournamentMetricCard(title: String(localized: "tournament.metric.live",     defaultValue: "مباشرة"),   value: viewModel.runningCount,   color: E360Color.live)
            TournamentMetricCard(title: String(localized: "tournament.metric.upcoming", defaultValue: "قادمة"),    value: viewModel.scheduledCount, color: E360Color.accent)
        }
    }
    ...
    private var allTournamentsSection: some View {
        VStack(alignment: .leading, spacing: 14) {
            SectionTitle(
                title: String(localized: "tournament.section.all",     defaultValue: "كل البطولات"),
                subtitle: String(localized: "tournament.section.all.sub", defaultValue: "بحث وفرز سريع من قاعدة بيانات Esports360")
            )

            HStack(spacing: 10) {
                Image(systemName: "magnifyingglass").foregroundStyle(E360Color.textSecondary)
                TextField(
                    String(localized: "tournament.search.placeholder", defaultValue: "ابحث عن بطولة، لعبة، أو دوري"),
                    text: $searchText
                )
                .font(E360Font.body(13, weight: .semibold))
                .foregroundStyle(E360Color.textPrimary)
            ...
            if viewModel.isLoading && viewModel.tournaments.isEmpty {
                VStack(spacing: 10) {
                    ForEach(0..<4, id: \.self) { _ in TournamentSkeletonRow() }
                }
    ...
    private var filteredTournaments: [BackendTournamentDTO] {
        viewModel.tournaments
            ...
            .filter { tournament in
                ...
                let haystack = [tournament.name, tournament.leagueName, tournament.seriesName,
                    tournament.gameName, tournament.gameShortName, tournament.gameSummary,
                    tournament.location, tournament.tier]
...
private struct FeaturedTournamentHeroCard: View {
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: 7) {
                HStack(spacing: 8) {
                    ...
                    Text(tournament.displayTier)
                        .font(E360Font.mono(10, weight: .bold)).foregroundStyle(E360Color.gold).lineLimit(1)
                }
                Text(tournament.displayName)
                    .font(E360Font.display(18, weight: .black)).foregroundStyle(E360Color.textPrimary).lineLimit(2)
                Text("\(tournament.displayGame) · \(tournament.location ?? "Global")")
                    .font(E360Font.body(12, weight: .semibold)).foregroundStyle(E360Color.textSecondary).lineLimit(1)
            }
            Spacer(minLength: 0)
        ...
        .overlay(RoundedRectangle(cornerRadius: 18, style: .continuous).stroke(E360Color.gold.opacity(0.22), lineWidth: 1))
    }
}
...
private struct TournamentCatalogCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            HStack(spacing: 8) {
                InfoPill(title: String(localized: "tournament.prize", defaultValue: "جائزة"), value: tournament.displayPrize, color: E360Color.gold)
                InfoPill(title: String(localized: "tournament.tier",  defaultValue: "نوع"),   value: tournament.displayTier,  color: E360Color.primary)
            }
            if let dateText = tournament.dateRangeText {
                Label(dateText, systemImage: "calendar")
                    .font(E360Font.body(11, weight: .semibold)).foregroundStyle(E360Color.textSecondary).lineLimit(1)
            }
        }
        .padding(16)
...
private struct TournamentListRow: View {
    ...
    var body: some View {
        HStack(spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 7) {
                Text(tournament.displayName)
                    .font(E360Font.body(15, weight: .black)).foregroundStyle(E360Color.textPrimary).lineLimit(2)
                HStack(spacing: 8) {
                    Text(tournament.displayGame)
                    Text("·")
                    Text(tournament.displayPrize)
        ...
        .overlay(RoundedRectangle(cornerRadius: 16, style: .continuous).stroke(E360Color.divider, lineWidth: 1))
    }
}
...
private struct TournamentLogoView: View {
    ...
    var body: some View {
        ZStack {
            RoundedRectangle(cornerRadius: size * 0.24, style: .continuous).fill(E360Color.elevatedSurface)
            ESImageView(
        ...
        .overlay(RoundedRectangle(cornerRadius: size * 0.24, style: .continuous).stroke(E360Color.divider, lineWidth: 1))
    }
}
...
private struct TournamentStatusChip: View {
    let status: String?
    var body: some View {
        Text(title).font(E360Font.mono(10, weight: .black)).foregroundStyle(color)
...
private struct TournamentMetricCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text(ArabicNumberFormatter.localized(value)).font(E360Font.number(20, weight: .black)).foregroundStyle(color)
        }
...
private struct SectionTitle: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(title).font(E360Font.display(20, weight: .black)).foregroundStyle(E360Color.textPrimary)
            Text(subtitle).font(E360Font.body(12, weight: .semibold)).foregroundStyle(E360Color.textSecondary)
        }
    }
}
...
private struct EmptyTournamentState: View {
    var body: some View {
        VStack(spacing: 10) {
            Image(systemName: "trophy").font(.system(size: 22, weight: .bold)).foregroundStyle(E360Color.gold)
            Text(String(localized: "tournament.empty.title",    defaultValue: "لا توجد بطولات مطابقة"))
                .font(E360Font.body(14, weight: .black)).foregroundStyle(E360Color.textPrimary)
            Text(String(localized: "tournament.empty.subtitle", defaultValue: "جرّب فلتر آخر أو امسح البحث الحالي."))
